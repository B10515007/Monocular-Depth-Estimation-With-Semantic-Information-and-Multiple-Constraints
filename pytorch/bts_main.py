...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
        wandb.init(project="BTS", entity="b10515007",name="res2net101_v1b_mask4addloss4connectDCN")
        # wandb.init(project="BTS",id="36rt656x", resume="allow")
    ...
    while epoch < args.num_epochs:
        ...
        torch.cuda.empty_cache()
        # if epoch == 9:  
        #     print("UnFrezze Backbone")
        #     for param in model.module.encoder.parameters():
        #         param.requires_grad = True
        #     torch.cuda.empty_cache()
        #     optimizer.add_param_group({'params': model.module.encoder.parameters(), 'weight_decay': args.weight_decay})
        #     torch.cuda.empty_cache()
        for step, sample_batched in enumerate(dataloader.data):
            ...
            losslpg8x8 = silog_criterion.forward(lpg8x8, depth_gt, mask.to(torch.bool))
            losslpg4x4 = silog_criterion.forward(lpg4x4, depth_gt, mask.to(torch.bool))
            losslpg2x2 = silog_criterion.forward(lpg2x2, depth_gt, mask.to(torch.bool))
            lossreduc1x1 = silog_criterion.forward(reduc1x1, depth_gt, mask.to(torch.bool))
            
            totalLoss = loss + lossreduc1x1 + losslpg2x2 + losslpg4x4 + losslpg8x8
            totalLoss.backward()
            
            # lossreduc1x1.backward(retain_graph=True)
            # losslpg2x2.backward(retain_graph=True)
            # losslpg4x4.backward(retain_graph=True)
            # losslpg8x8.backward(retain_graph=True)
            ...
            if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                wandb.log({"loss": loss, "losslpg8x8":losslpg8x8,"losslpg4x4":losslpg4x4,"losslpg2x2":losslpg2x2,
                "lossreduc1x1":lossreduc1x1, "totalLoss":totalLoss},step = global_step)
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                    ...
                    # test_table.add_data(loss, current_lr, var_sum.item()/var_cnt)
                    # wandb.run.log_artifact(test_data_at) 

                    depth_gt = torch.where(depth_gt < 1e-3, depth_gt * 0 + 1e3, depth_gt)
                    # for i in range(num_log_images):
                        # wandb.log({'depth_gt/image/{}'.format(i): wandb.Image(normalize_result(1/depth_est[i, :, :, :].data))},step = step)
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded:
                ...
                wandb.log({'silog':eval_measures[0],
                ...
                'd1':eval_measures[6], 
                'd2':eval_measures[7],
                'd3':eval_measures[8]},step = global_step)
