...
class upconv(nn.Module):
    def __init__(self, in_channels, out_channels, ratio=2, dcn=True):
        super(upconv, self).__init__()
        self.elu = nn.ELU()
        
        if dcn == True:
            self.conv = DCN(in_channels=in_channels, out_channels=out_channels, bias=False, kernel_size=3, stride=1, padding=1)
        else:
...
class bts(nn.Module):
    def __init__(self, params, feat_out_channels, segModel, num_features=512):
        ...
        self.conv1      = torch.nn.Sequential(DCN(num_features // 16 + 4+4, num_features // 16, 3, 1, 1, bias=False),
                                              nn.ELU())
        self.get_depth  = torch.nn.Sequential(DCN(num_features // 16, 1, 3, 1, 1, bias=False),
                                              nn.Sigmoid())
        self.segModel = segModel
        self.ninteentoone = torch.nn.Sequential(nn.Conv2d(19, 32, 3, 1, 1, bias=False), nn.Conv2d(32, 1, 3, 1, 1, bias=False))
        # self.scale18 = torch.nn.Sequential(upconv(1,1,0.125))        
        self.scale12 = torch.nn.Sequential(upconv(19,32, 0.5, dcn=False), nn.Conv2d(32, 1, 3, 1, 1, bias=False))
        self.scaleBig2 = torch.nn.Sequential(upconv(19,32, 2, dcn=False), nn.Conv2d(32, 1, 3, 1, 1, bias=False))
        self.scaleBig4 = torch.nn.Sequential(upconv(19,32, 4, dcn=False), nn.Conv2d(32, 1, 3, 1, 1, bias=False)) 
        self.conup_x12 = upconv(1,1, 8, dcn=False)
        self.conup_x0 = upconv(1,1, 4, dcn=False)
        self.conup_x2 = upconv(1,1, 2, dcn=False)
        self.conup_x4 = upconv(1,1, 1, dcn=False)
    def forward(self, features, focal):
        ...
        outputs = {}
        outputs['res2'] = features[2]
        outputs['res3'] = features[3]
        outputs['res4'] = features[4]
        outputs['res5'] = features[5]

        segfeature = self.segModel(outputs)
        # up_x = torch_nn_func.interpolate(segfeature, scale_factor=4, mode='nearest')
        segfeatureO = self.ninteentoone(segfeature)
        up_xO = torch_nn_func.interpolate(segfeatureO, scale_factor=4, mode='nearest')

        # segfeature18 = self.ninteentoone(self.scale18(segfeature))        
        segfeature12 = self.scale12(segfeature)
        up_x12 = torch_nn_func.interpolate(segfeature12, scale_factor=8, mode='nearest')

        segfeature2 = self.scaleBig2(segfeature)
        up_x2 = torch_nn_func.interpolate(segfeature2, scale_factor=2, mode='nearest')

        segfeature4 = self.scaleBig4(segfeature)
        up_x4 = torch_nn_func.interpolate(segfeature4, scale_factor=1, mode='nearest')

        # segfeature = self.bigseg(segfeature)
        # print(segfeature.shape)
        # return
        ...
        concat4_daspp = concat4_daspp + segfeature12
        ...
        # print('red',reduc1x1.shape)
        # return
        concat1 = torch.cat([upconv1, reduc1x1, depth_2x2_scaled, depth_4x4_scaled, depth_8x8_scaled,up_x12,up_xO,up_x2,up_x4], dim=1)
        concat1 = concat1 + segfeature4
        iconv1 = self.conv1(concat1)
        final_depth = self.params.max_depth * self.get_depth(iconv1)
        depth_8x8_scaled = depth_8x8_scaled * self.params.max_depth
        depth_4x4_scaled = depth_4x4_scaled * self.params.max_depth
        depth_2x2_scaled = depth_2x2_scaled * self.params.max_depth
        reduc1x1 = reduc1x1 * self.params.max_depth
        if self.params.dataset == 'kitti':
            final_depth = final_depth * focal.view(-1, 1, 1, 1).float() / 715.0873 #訓練用 final_depth = final_depth * focal.view(-1, 1, 1, 1).float() / 715.0873 
            depth_8x8_scaled = depth_8x8_scaled * self.params.max_depth * focal.view(-1, 1, 1, 1).float() / 715.0873
            depth_4x4_scaled = depth_4x4_scaled * self.params.max_depth * focal.view(-1, 1, 1, 1).float() / 715.0873
            depth_2x2_scaled = depth_2x2_scaled * self.params.max_depth * focal.view(-1, 1, 1, 1).float() / 715.0873
            reduc1x1 = reduc1x1 * self.params.max_depth * focal.view(-1, 1, 1, 1).float() / 715.0873
            #inference final_depth = final_depth * focal / 715.0873
...
class encoder(nn.Module):
    def __init__(self, params):
        ...
        if params.encoder == 'densenet121_bts':
            ...
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        elif params.encoder == 'res2net101_bts':
            self.base_model = res2net101_26w_4s(pretrained=True)
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        elif params.encoder == 'res2net101_v1b_bts':
            self.base_model = res2net101_v1b(pretrained=True)
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        elif params.encoder == 'res2net50_v1b_bts':
            self.base_model = res2net50_v1b(pretrained=True)
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        else:
