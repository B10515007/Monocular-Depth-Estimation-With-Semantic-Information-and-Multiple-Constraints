...
class upconv(nn.Module):
    def __init__(self, in_channels, out_channels, ratio=2, dcn=False):
...
class bts(nn.Module):
    def __init__(self, params, feat_out_channels, segModel, num_features=512):
        ...
        self.ninteentoone = torch.nn.Sequential(nn.Conv2d(40, 1, 3, 1, 1, bias=False))
        # self.scale18 = torch.nn.Sequential(upconv(1,1,0.125))        
        self.scale12 = torch.nn.Sequential(upconv(40,40, 0.5, dcn=False), nn.Conv2d(40, 1, 3, 1, 1, bias=False))
        self.scaleBig2 = torch.nn.Sequential(upconv(40,40, 2, dcn=False), nn.Conv2d(40, 1, 3, 1, 1, bias=False))
        self.scaleBig4 = torch.nn.Sequential(upconv(40,40, 4, dcn=False), nn.Conv2d(40, 1, 3, 1, 1, bias=False)) 
    ...
    def forward(self, features, focal):
        ...
        # up_x = torch_nn_func.interpolate(segfeatureO, scale_factor=4, mode='nearest')
